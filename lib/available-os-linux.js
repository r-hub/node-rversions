...
async function available_os_linux_posit(os, cache, osdata, arch) {
    ...
    const distro = osdata.family || os.split('-')[0];
    const fn = linux_filenames.posit[distro];
    const pkgpath = osdata['pkg-path'] !== undefined ? osdata['pkg-path'] : 'pkgs/';
    ...
    var av = res[1].map(function(v) {
        ...
        return {
            ...
            url: "https://cdn.posit.co/r/" + os + "/" + pkgpath +
    ...
    // istanbul ignore else: all platforms have these keys
    if (!!osdata[key]) {
...
async function available_os_linux(os, arch, cache = true) {
    ...
    // istanbul ignore else: already checked above
