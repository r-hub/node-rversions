...
async function available_os_linux_x86_64(os, cache) {
    ...
    var res = await Promise.all([pbar, pbld]);
...
async function available_os_linux_arm64(os, cache) {
    ...
    var all = await Promise.all([pbar, pbld]);
    const rvers = all[0];
    ...
    av = av.sort(function(a, b) {
        ...
        if (a == "devel") { a = "1000.0.0"; }
        if (b == "devel") { b = "1000.0.0"; }
        if (a == "next") { a = "999.0.0"; }
        if (b == "next") { b = "999.0.0"; }
...
async function available_os_linux(os, arch, cache = true) {
    ...
    if (arch == "x86_64") {
        return available_os_linux_x86_64(os, cache);
    } else if (arch == "arm64") {
        return available_os_linux_arm64(os, cache);
    }
