...
rtools_versions_version_aarch64 = rtools_versions_aarch64.map(x => { return x.version; });


function find_rtools_record_x86_64(version) {
    const idx = rtools_versions_version.indexOf(version);
    /* istanbul ignore if */
    if (idx == -1) {
        throw new Error("Unknown rtools version: '" + version + "'");
    }
    return rtools_versions[idx];
}

function find_rtools_record_arm64(version) {
    const idx = rtools_versions_version_aarch64.indexOf(version);
    /* istanbul ignore if */
    if (idx == -1) {
        throw new Error("Unknown rtools version: '" + version + "'");
    }
    return rtools_versions_aarch64[idx];
}

function find_rtools_record(version, arch = "x86_64") {
    if (arch == "x86_64") {
        return find_rtools_record_x86_64(version);
    } else {
        return find_rtools_record_arm64(version);
    }
}

function rtools_match(version, arch = "x86_64") {
    var ver, url;
    if (semver.lt(version, "2.6.0")) {
        ...
        ver = "35";
    } else if (semver.lt(version, "4.2.0")) {
        ver = "40";
    } else if (semver.lt(version, "4.3.0")) {
        ver = "42";
    } else if (semver.lt(version, "4.4.0")) {
        ver = "43";
    } else if (semver.lt(version, "4.5.0")) {
        ver = "44";
    } else {
	ver = "45";
    }

    return find_rtools_record(ver, arch);
}

module.exports = {
    rtools_versions: rtools_versions,
    rtools_versions_aarch64: rtools_versions_aarch64,
