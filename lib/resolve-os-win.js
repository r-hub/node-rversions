...
function win_url(version) {
    if (semver.lt(version, "3.6.0")) {
        return(
...
async function resolve_os_win(version, arch, cache) {
    ...
    if (version == "devel") {
        const devel = await r_devel_win(cache);
        ver["version"]  = devel["version"];
        ver["nickname"] = devel["nickname"];
        ver["type"]     = "devel";
        ver["url"]      = "https://cloud.r-project.org/bin/windows/base/R-devel-win.exe";
        ver["date"]     = devel["date"];

    } else if (version == "next") {
        const next = await r_next_win(cache);
        ...
        const rel = await r_release_win(cache);
    ...
    const rtools = rtools_match(ver.version);
