...
function get_release_url_x86_64(version, sv) {
    ...
    } else if (semver.lt(sv, "3.1.0")) {
        return `https://cran-archive.r-project.org/bin/macosx/base/R-${version}.pkg`;
    } else if (semver.lt(sv, "3.2.0")) {
    ...
    } else if (semver.lte(sv, "3.3.3")) {
        return `https://cran-archive.r-project.org/bin/macosx/base/R-${version}.pkg`;
    } else if (semver.lte(sv, "3.6.3")) {
        return `https://cran-archive.r-project.org/bin/macosx/el-capitan/base/R-${version}.pkg`;
    } else if (semver.lte(sv, "4.2.3")) {
...
function get_release_url(version, sv, arch) {
    if (arch == "x86_64") {
        return get_release_url_x86_64(version, sv);
    } else {
        return get_release_url_arm64(version, sv);
    }
}
...
async function available_os_mac(arch = "x86_64", cache = true) {
    ...
    av = av.map(function(v) {
        v.name = v.version;
        v.type = "release";
        v.url = get_release_url(v.version, v.semver, arch);
        return v;
    })
