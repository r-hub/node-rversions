...
async function resolve_os_linux_x86_64(version, os, cache) {
    ...
    if (version == "devel") {
        ...
        if (rver === undefined) {
            throw "Cannot find R release Linux build for arch 'x86_64'.";
        }
        ...
        if (rel === undefined) {
            throw "Could not find any Linux builds."
        }
...
async function resolve_os_linux_arm64(version, os, cache) {
    ...
    if (version == "devel") {
        ...
        ver["nickname"] = oldrel["nickname"];
        ver["type"]     = "oldrel/" + which;
        ver["url" ]     = url.replaceAll("{}", ver["version"]);
        ver["date"]     = oldrel["date"];
...
async function resolve_os_linux(version, os, arch, cache) {
    ...
    if (os == undefined) {
        throw "Unknown Linux distro: '" + oos + "'.";
    }

    if (arch == "x86_64") {
        return resolve_os_linux_x86_64(version, os, cache);
    } else if (arch == "arm64") {
        return resolve_os_linux_arm64(version, os, cache);
    }
}
