...
async function resolve_os_linux_x86_64(version, os, cache) {
    ...
    ver["ppm-binaries"] = false;
    ver["ppm-binary-url"] = null;
    for (i in linux_map) {
        const dist = linux_map[i];
        if (dist.id == os) {
            ver["ppm-binaries"] = dist["ppm-binaries"] || false;
            ver["ppm-binary-url"] = dist["ppm-binary-url"] || null;
            break;
        }
    }
