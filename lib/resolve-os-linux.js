...
async function resolve_os_linux_arch(version, os, cache, arch, platform) {
    var ver = {};
    let distro = platform.family || os.split('-')[0];
    let release = os.split('-')[1];
    let aarch = arch == "amd64" ? "x86_64" : "aarch64";
    const pkgpath = platform['pkg-path'] !== undefined ? platform['pkg-path'] : 'pkgs/';
    const positurl = "https://cdn.posit.co/r/" + os + "/" + pkgpath +
        linux_filenames.posit[distro]
            .replaceAll("{os}", os)
            .replaceAll("{arm64}", arch == "arm64" ? "-arm64" : "")
            .replaceAll("{arch}", arch)
            .replaceAll("{aarch}", aarch);
    const ghurl = linux_filenames.github[distro] === undefined ? undefined :
        linux_filenames.github[distro]
            .replaceAll("{rel}", release)
            .replaceAll("{arch}", arch)
            .replaceAll("{aarch}", aarch);

    if (arch == "amd64") {
        ver["ppm-binaries"] = platform["ppm-binaries"] || false;
        ver["ppm-binary-url"] = platform["ppm-binary-url"] || null;
    } else {
        ver["ppm-binaries"] = platform["aarch64-ppm-binaries"] || false;
        ver["ppm-binary-url"] =
            ver["ppm-binaries"] ? platform["ppm-binary-url"] : null;
    }
    ...
    if (arch == "amd64") {
        ...
        if (platform && platform.aarch64 === true) {
            url = positurl;
        } else if (platform && platform.aarch64 === "rhub") {
            url = ghurl;
        } else {
            throw new Error("No R builds available for " + os + " aarch64.");
        }
    }
    ...
    if (version == "devel") {
        ...
        ver["type"]     = "devel";
        ver["url"]      = url.replaceAll("{ver}", "devel");
        ver["date"]     = devel["date"];
        ...
        ver["url"]      = url.replaceAll("{ver}", "next");
        ver["date"]     = next["date"];

    } else if (
            version == "release" &&
            (arch == "amd64" || platform.aarch64 === true)) {
        const p1 = linux_builds_posit(cache);
        ...
        if (rver === undefined) {
            // istanbul ignore next: can only happen if a platform has no builds
            throw new Error("Cannot find R release Linux build for arch '" + arch + "'.");
        }

        ver["version"] = rver.version;
        ver["nickname"] = await get_nick(rver.version);
        ver["type"] = "release";
        ver["url"]  = url.replaceAll("{ver}", rver.version);
        ver["date"] = rver.date;

    } else if (version == "release" && platform.aarch64 === 'rhub') {
        ...
        ver["url" ]     = url.replaceAll("{ver}", ver["version"]);
        ...
        // If there is no build yet, then we use the latest
        // build instead
    ...
    } else if (semver.valid(version + ".0") && platform.aarch64 === 'rhub') {
...
async function resolve_os_linux(version, os, arch, cache) {
    ...
    return resolve_os_linux_arch(version, os2, cache, arch, platform);
}
